...
if($_POST['method'] =='edit-service')
{
	// $importtype=$_POST['importtype'];
	$error = FALSE;

	$errors = array();
    $required = array('service_no','date', 'return_code');
    foreach ($required as $field) 
    {
        if(empty($_POST[$field]))
        {
    ...
    else
    {
    	//exit;
    	$service_no   = mysqli_real_escape_string($conn, trim($_POST['service_no']));
		$return_code  = mysqli_real_escape_string($conn, trim($_POST['return_code']));

		// $select = mysqli_query($conn, "SELECT * FROM ss_item_stk WHERE code = '".$code[$i]."'");
		// if(mysqli_num_rows($select) == 0)
		// {

			$qry = mysqli_query($conn, "UPDATE `ss_service_stk` SET `service_no` = '".$service_no."', `return_code` = '".$return_code."',`progress` = '2', `status` = '".$_POST['pstatus']."',`update_date` = NOW() WHERE `id` = '".$_POST['id']."'");
		// }
    	// echo $k;
    	// exit;
    	if($qry)
