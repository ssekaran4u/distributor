...
if($_POST['method'] =='add-form')
{
    ...
    if($importtype != 1)
    {
    	array_push($required, "cid","pid","code","price","allowance","sta","d_allowance");
    }
								...
								if($sel_4->avl_lmt > $_tot)
								{
									...
									if($row_3 > 0)
									{
										...
										{	
											...
											$ins_1 = mysqli_query($conn, "UPDATE `ss_distributor_inv_details` SET `qty` = '".$t_qty."', `total_cost` = '".$t_tot."' WHERE `so_id` = '".$so_re."' AND `pid` = '".$sel_3->id."'");
										}
										else
										{	
			...
			if($rsrow==0)
			{
				...
				if($avl_lmt > $_tot)
				{
					...
					if($num_1 > 0)
					{
						...
						$ins_1 = "INSERT INTO `ss_distributor_inv_details`(`so_id`, `so_no`, `customer_id`, `cid`, `pid`, `code`, `hsn`, `qty`, `price`, `gst`, `allowance`, `sta`, `d_allowance`, `discount`, `total_cost`, `createdate`) VALUES ('".$so_id."','".$so_no."','".$customer_id."','".$cid."','".$pid."', '".$description."','".$hsn."','".$qty."','".$price."','".$gst."','".$allowance."','".$sta."','".$d_allowance."','".$discount."', '".$_tot."',NOW())";
						...
						{	
							...
							for($i = 0 ; $i < $qty ; $i++)
							{
								...
								$up_stk = mysqli_query($conn, "UPDATE `ss_item_stk` SET `dist_id`='".$customer_id."',`sales_no` = '".$so_no."', `status` = '0' WHERE `id` = '".$code[$i]."' ");
							}
					...
					else
					{
						...
						$ins_3 = "INSERT INTO `ss_distributor_inv_details`(`so_id`, `so_no`, `customer_id`, `cid`, `pid`, `code`, `hsn`, `qty`, `price`, `gst`, `allowance`, `sta`, `d_allowance`, `discount`, `total_cost`, `createdate`) VALUES ('".$so_re."','".$so_no."','".$customer_id."','".$cid."','".$pid."', '".$description."','".$hsn."','".$qty."','".$price."','".$gst."','".$allowance."','".$sta."','".$d_allowance."','".$discount."', '".$_tot."',NOW())";
...
elseif($_POST['method'] =='edit-form')
{
	...
	if($customer_id!='' && $dated!='' && $cid!='' && $pid!='' && $code!='' && $qty!='')
	{
		...
		if($e_avl_lmt > $_tot)
		{
			...
			$ins_1 = "UPDATE `ss_distributor_inv_details` SET `customer_id`= '".$customer_id."', `cid`= '".$cid."', `pid`= '".$pid."',`code`= '".$description."',`hsn`= '".$hsn."',`qty`= '".$qty."', `price`= '".$price."', `gst`= '".$gst."', `allowance`= '".$allowance."',`sta`= '".$sta."',`d_allowance`= '".$d_allowance."',`discount`= '".$discount."',`total_cost`= '".$_tot."',`updatedate`= NOW() WHERE `auto_id` = '".$auto_id."'";

			$up_stk = mysqli_query($conn, "UPDATE `ss_item_stk` SET `dist_id`=NULL,`sales_no` = NULL ,`status` = '1' WHERE `sales_no` = '".$so_no."' AND `cid`= '".$cid."' AND `product_id`= '".$pid."'");
			if(mysqli_query($conn, $ins_1))
			{	
