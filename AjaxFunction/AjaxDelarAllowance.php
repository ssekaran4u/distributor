...
if($_POST['method'] =='add-allowance')
{
	...
	$exits = "SELECT * FROM `ss_delar_allowance` WHERE `description` = '".$description."' AND `published` = '1'";
	...
	if($rsrow==0)
	{
		if($description!='')
		{
			$qry = "INSERT INTO `ss_delar_allowance`(`description`, `type`, `value`,`createdate`) VALUES ('".$description."' ,'".$type."', '".$value."', NOW())"; 
...
elseif($_POST['method'] =='edit-allowance')
{
    ...
    if($data)
    {
    	...
    	{
    		$qry = "UPDATE `ss_delar_allowance` SET `description` = '".$description."',
    			`type` = '".$type."',
    			`value` = '".$value."',
