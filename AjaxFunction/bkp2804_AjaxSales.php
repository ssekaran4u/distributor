						...
						if($row_1)
						{
							...
							if($sel_4->avl_lmt > $last_tot)
							{
								...
								if($row_3 > 0)
								{
									...
									$sel_7 = mysqli_query($conn, "SELECT `pid` FROM ss_sales_details WHERE `so_id` = '".$so_id."' AND `pid` = '".$sel_3->id."'");

									$row_7 = mysqli_num_rows($sel_7);

									if($row_7 > 0)
									{
										$sel_8 = mysqli_fetch_object(mysqli_query($conn, "SELECT SUM(`qty`) AS `qty` FROM ss_sales_details WHERE `so_id` = '".$so_id."' AND `pid` = '".$sel_3->id."'"));
										...
										$ins_1 = mysqli_query($conn, "UPDATE `ss_sales_details` SET `qty` = '".$t_qty."', `total_cost` = '".$last_tot."' WHERE `so_id` = '".$so_re."' AND `pid` = '".$sel_3->id."'");
									}
									else
									{

										$ins_3 = mysqli_query($conn, "INSERT INTO `ss_sales_details`(`so_id`, `so_no`, `customer_id`, `cid`, `pid`, `code`, `hsn`, `qty`, `price`, `gst`, `allowance`, `sta`, `d_allowance`, `total_cost`, `createdate`) VALUES ('".$so_re."','".$so_no."','".$customer_id."','".$sel_3->cid."','".$sel_3->id."', '".$sel_3->description."','".$sel_3->hsn."','".$qty."','".$sel_3->oprice."','".$sel_3->gst."','".$sel_3->allowance."','".$sel_3->sta."','".$sel_4->d_allowance."', '".$last_tot."',NOW())");	
									}
