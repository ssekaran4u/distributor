...
if($_POST['method'] =='edit-payment')
{
	...
	if($amount!='' && $date!='')
	{
		if($balance >= $amount)
		{
			$cre_lmt = mysqli_fetch_object(mysqli_query($conn, "SELECT `avl_lmt` FROM `ss_distributors` WHERE `id` = '".$customer_id."' AND deleted = '1' AND status = '1'"));

			$up_valu = $cre_lmt->avl_lmt + $amount;

			$qry_2 = mysqli_query($conn, "UPDATE `ss_distributors` SET `avl_lmt` = '".$up_valu."' WHERE id = '".$customer_id."'");

			$qry = mysqli_query($conn, "INSERT INTO `ss_distributor_payment`(`customer_id`, `amount`, `type`, `description`, `date`, `createdate`) VALUES ('".$customer_id."','".$amount."','".$type."','".$description."','".$date."', NOW())");
			if($qry)
			{
