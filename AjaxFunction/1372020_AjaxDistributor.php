...
{
	// var_dump($_POST['store']);die();
	$error = FALSE;
	$errors = array();
    $required = array('store','owner','state_id','city_id','address','email','mobile','amobile','credit_lmt','avl_lmt','gst_no','d_allowance');
    ...
    if($_POST['method']=="add-customer")
    {
    	...
    	$qry1 = "INSERT INTO `ss_distributors`(`disno`,`store`, `owner`, `state`, `location`, `customeraddress`, `gst`,`pan_no`, `email_id`, `mobile`, `amobile`, `credit_lmt`, `pre_lmt`, `avl_lmt`, `d_allowance`, `password`,`createdate`) VALUES ('".$disno."','".$store."','".$owner."','".$state_id."','".$city_id."','".$address."','".$gst_no."','".$pan_no."','".$email."','".$mobile."','".$amobile."','".$credit_lmt."','".$credit_lmt."','".$credit_lmt."','".$d_allowance."','".$password."',NOW())";
    ...
    else
    {
    	$editid = $_POST['editid'];
    	$status = $_POST['status'];
    	$qry2 = "UPDATE `ss_distributors` SET `store`='".$store."',`owner`='".$owner."',`state`='".$state_id."',`location`='".$city_id."',`customeraddress`='".$address."',`gst`='".$gst_no."',`pan_no`='".$pan_no."',`mobile`='".$mobile."',`amobile`='".$amobile."',`credit_lmt`='".$credit_lmt."',`pre_lmt`='".$pre_lmt."',`avl_lmt`='".$avl_lmt."',`d_allowance`='".$d_allowance."',";
    	if($_POST['password']!='')
    	{
    		$qry2 .= " `password`='".$password."', " ;
    	}
    	$qry2 .= " `status`='".$status."',`updatedate`=NOW() WHERE `id` = '".$editid."'";
...
elseif($_POST['method'] =='edit-customer')
{
    ...
    if($data)
    {
    	if($name!='' && $address!='' && $email!='' && $mobile && $credit_lmt!='' && $cname!='' && $state_id!='' && $city_id!='' && $pincode!= '')if($name!='' || $address!='' || $email!='' || $mobile != '')
    	{
    		...
    		$qry = "UPDATE `ss_customers` SET `name`= '".$name."',`address`= '".$address."',`state`= '".$state_id."',`city`= '".$city_id."',`pincode`= '".$pincode."',`email`= '".$email."',`cname`= '".$cname."',`gst_no`= '".$gst_no."',`d_allowance`= '".$d_allowa."',`phone`= '".$almobile."',`mobile`= '".$mobile."',`credit_lmt`='".$credit_lmt."', `pre_lmt` = '".$credit_lmt."', `avl_lmt` = '".$avi_lmt."',`updatedate`= NOW() WHERE id = '".$id."'";
