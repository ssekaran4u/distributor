...
if($_POST['method'] =='add-customer')
{
	...
	if($rsrow==0)
	{
		if($name!='' && $address!='' && $email!='' && $mobile && $credit_lmt!='' && $cname!='' && $state_id!='' && $city_id!='' && $pincode!= '' && $excutive!='')
		{
			$qry = "INSERT INTO `ss_customers`(`userno`,`userid`,`usertype`,`name`, `address`, `state`, `city`,`pincode`, `email`, `cname`, `gst_no`, `d_allowance`, `excutive`,`phone`, `mobile`, `credit_lmt`, `pre_lmt`, `avl_lmt`,`status`,`createdate`) VALUES ('".$userno."','".$uid."','".$_SESSION['type']."','".$name."','".$address."','".$state_id."','".$city_id."','".$pincode."','".$email."','".$cname."','".$gst_no."', '".$d_allowa."', '".$excutive."','".$almobile."','".$mobile."', '".$credit_lmt."', '".$credit_lmt."', '".$credit_lmt."', '0', NOW())"; 
		...
		{
			$response['status']=false;
			$response['message']="<strong>Oops ! </strong>Fill Your Sales Excutive";
	        echo json_encode($response);
		}
