...
if($_POST['method'] =='add-form')
{
	...
	else
	{
		if($importtype == 1)
    	{
			...
			$qry_1 = mysqli_fetch_object(mysqli_query($conn, "SELECT * FROM `ss_distributors` WHERE `id` = '".$customer_id."' AND `deleted` = '1' "));
			$excutive = !empty($qry_1->excutive)?$qry_1->excutive:'';
				...
				if($fileData)
				{
					...
					foreach($datas as $rawRow)
					{
						...
						if($row_1)
						{
							...
							if($cus_type == '2')
							{	
								$upt_1 = mysqli_query($conn, "UPDATE `ss_item_stk` SET `dist_id`='".$customer_id."',`sales_no` = '".$so_no."' , `sales_date` = '".date('Y-m-d', strtotime($dated))."', `status` = '0', `bill_status` = '0' WHERE `code` = '".$code."' ");
							}
							else
							{	
								$upt_1 = mysqli_query($conn, "UPDATE `ss_item_stk` SET `dist_id`='".$customer_id."',`sales_no` = '".$so_no."' , `sales_date` = '".date('Y-m-d', strtotime($dated))."', `status` = '0', `bill_status` = '0', `delar_status` = '0' WHERE `code` = '".$code."' ");
							}
							...
							// {
								...
								$sel_5 = mysqli_fetch_object(mysqli_query($conn, "SELECT * FROM `ss_distributor_price` WHERE `distributor_id` = '".$customer_id."' AND `product_id` = '".$product_id."' AND `status` = '1' AND `published` = '1' ORDER BY `id` DESC "));
								...
								if($avl_lmt > $last_tot)
								{
									...
									if($row_3 > 0)
									{
										...
										$sel_7 = mysqli_query($conn, "SELECT `pid` FROM ss_distributor_inv_details WHERE `so_id` = '".$so_id."' AND `pid` = '".$pro_id."'");

										$row_7 = mysqli_num_rows($sel_7);
										if($row_7 > 0)
										{	
											$sel_8 = mysqli_fetch_object(mysqli_query($conn, "SELECT SUM(`qty`) AS `qty` FROM ss_distributor_inv_details WHERE `so_id` = '".$so_id."' AND `pid` = '".$pro_id."'"));
											...
											$ins_1 = mysqli_query($conn, "UPDATE `ss_distributor_inv_details` SET `qty` = '".$t_qty."', `total_cost` = '".$last_tot."' WHERE `so_id` = '".$so_re."' AND `pid` = '".$pro_id."'");
										}
										else
										{	
											$ins_1 = mysqli_query($conn, "INSERT INTO `ss_distributor_inv_details`(`so_id`, `so_no`, `customer_id`, `cid`, `pid`, `code`, `hsn`, `qty`, `price`, `gst`, `allowance`, `sta`, `d_allowance`, `total_cost`, `dated`, `createdate`) VALUES ('".$so_re."','".$so_no."','".$customer_id."','".$cid."','".$pro_id."', '".$description."','".$hsn."','".$qty."','".$price_val."','".$gst."','".$allowance."','".$sta."','".$d_allowance."', '".$last_tot."', '".date('Y-m-d', strtotime($dated))."', NOW())");
										}
									...
									{
										$ins_2 = mysqli_query($conn, "INSERT INTO `ss_distributor_invoice`(`userid`,`so_no`, `customer_id`, `excutive`, `tcs_type`, `order_date`, `order_no`, `dated`, `docu_no`, `despatched`, `acad_id`, `createdate`) VALUES ('".$_SESSION['uid']."','".$so_no."','".$customer_id."', '".$excutive."', '".$tcs_type."', '".date('Y-m-d')."','".$order_no."','".date('Y-m-d', strtotime($dated))."','".$docu_no."', '".$despatched."', '".$_SESSION['acad_year']."', NOW())");
										$so_re = mysqli_insert_id($conn);

										$ins_3 = mysqli_query($conn, "INSERT INTO `ss_distributor_inv_details`(`so_id`, `so_no`, `customer_id`, `cid`, `pid`, `code`, `hsn`, `qty`, `price`, `gst`, `allowance`, `sta`, `d_allowance`, `total_cost`, `dated`, `createdate`) VALUES ('".$so_re."','".$so_no."','".$customer_id."','".$cid."','".$pro_id."', '".$description."','".$hsn."','".$qty."','".$price_val."','".$gst."','".$allowance."','".$sta."','".$d_allowance."', '".$last_tot."', '".date('Y-m-d', strtotime($dated))."', NOW())");
		...
		else
		{
			...
			$sel_1 = mysqli_fetch_object(mysqli_query($conn, "SELECT * FROM `ss_distributors` WHERE `id` = '".$customer_id."' AND `deleted` = '1' "));
			$excutive = !empty($sel_1->excutive)?$sel_1->excutive:'';
			// print_r($code); 
			...
			if($rsrow==0)
			{
				...
				if($avl_lmt > $last_tot)
				{
					...
					if($num_1 > 0)
					{
						...
						$ins_1 = "INSERT INTO `ss_distributor_inv_details`(`so_id`, `so_no`, `customer_id`, `cid`, `pid`, `code`, `hsn`, `qty`, `price`, `gst`, `allowance`, `sta`, `d_allowance`, `discount`, `total_cost`, `dated`, `createdate`) VALUES ('".$so_id."','".$so_no."','".$customer_id."','".$cid."','".$pid."', '".$description."','".$hsn."','".$qty."','".$price."','".$gst."','".$allowance."','".$sta."','".$d_allowance."','".$discount."', '".$last_tot."', '".date('Y-m-d', strtotime($dated))."', NOW())";
					...
					else
					{
						...
						$ins_3 = "INSERT INTO `ss_distributor_inv_details`(`so_id`, `so_no`, `customer_id`, `cid`, `pid`, `code`, `hsn`, `qty`, `price`, `gst`, `allowance`, `sta`, `d_allowance`, `discount`, `total_cost`, `dated`, `createdate`) VALUES ('".$so_re."','".$so_no."','".$customer_id."','".$cid."','".$pid."', '".$description."','".$hsn."','".$qty."','".$price."','".$gst."','".$allowance."','".$sta."','".$d_allowance."','".$discount."', '".$last_tot."', '".date('Y-m-d', strtotime($dated))."', NOW())";
...
elseif($_POST['method'] =='edit-form')
{
	...
	if($customer_id!='' && $dated!='' && $cid!='' && $pid!='' && $code!='' && $qty!='')
	{
		...
		if($e_avl_lmt > $last_tot)
		{
			...
			$ins_1 = "UPDATE `ss_distributor_inv_details` SET `customer_id`= '".$customer_id."', `cid`= '".$cid."', `pid`= '".$pid."',`code`= '".$description."',`hsn`= '".$hsn."',`qty`= '".$qty."', `price`= '".$price."', `gst`= '".$gst."', `allowance`= '".$allowance."',`sta`= '".$sta."',`d_allowance`= '".$d_allowance."',`discount`= '".$discount."',`total_cost`= '".$last_tot."', `dated` = '".date('Y-m-d', strtotime($dated))."',`updatedate`= NOW() WHERE `auto_id` = '".$auto_id."'";
