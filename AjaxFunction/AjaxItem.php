...
if($_POST['method'] =='add-item')
{
	...
	if($rsrow==0)
	{
		if($hsn!='' && $title!='' && $c_id!='' && $b_id!='' && $description!='' && $price!='' && $oprice!='')
		{
			$qry = "INSERT INTO `ss_items`(`pcode`,`title`, `hsn`, `brand`, `cid`, `description`, `gst`, `price`, `oprice`, `extra`, `allowance`, `sta`,`createdate`) VALUES ('".$pcode."','".$title."', '".$hsn."', '".$b_id."', '".$c_id."', '".$description."', '".$gst."', '".$price."', '".$oprice."', '0', '".$allowance."', '".$sta."',NOW())"; 
	 ...
	 //        echo json_encode($response);
		...
		{
			$response['status']=false;
			$response['message']="<strong>Oops ! </strong>Fill Your MRP";
	        echo json_encode($response);
		}
		elseif($oprice =='')
		{
