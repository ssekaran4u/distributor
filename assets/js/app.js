...
//logout
    $(document).ready(function(){
        ...
        function getFrequentOrders () {
            $.ajax({
              url       : 'inc/frequentOrders.php',
              method    : 'POST',
            }).done(function (response) {
                var df = response.split('#');
                if(df[0]==1)
                {    
                    if(df[1]>0)
                    {
                        $('#_nadge').show();
                        $('#_nadge').html(df[1]+' New');
                    }
                    if(df[2]>0)
                    {
                        $('#_nadge_2').show();
                        $('#_nadge_2').html(df[2]+' New');
                    }
                }
            });
        }
       // setInterval(getFrequentOrders, 900);
      	function getFrequentOrders () {
            $.ajax({
              url       : 'inc/count.php',
              method    : 'POST',
        ...
        //setInterval(getFrequentOrders, 100000);
