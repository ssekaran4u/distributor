...
$qry_1 = mysqli_fetch_object(mysqli_query($conn, "SELECT * FROM `ss_distributors` WHERE id = '".$cus_id."'"));

$owner      = !empty($qry_1->owner)?$qry_1->owner:'---';
$store      = !empty($qry_1->store)?$qry_1->store:'---';
$address    = !empty($qry_1->customeraddress)?$qry_1->customeraddress:'---';
$mobile     = !empty($qry_1->mobile)?$qry_1->mobile:'---';
$gst_number = !empty($qry_1->gst)?$qry_1->gst:'---';
$tcs_type   = !empty($qry_1->tcs_type)?$qry_1->tcs_type:'---';
$tcs_no     = !empty($qry_1->tcs_no)?$qry_1->tcs_no:'---';
$state      = !empty($qry_1->state)?$qry_1->state:'---';
$pan_card   = !empty($qry_1->pan)?$qry_1->pan:'---';
$excutive   = !empty($qry_1->excutive)?$qry_1->excutive:'---';

$excut_qry  = mysqli_fetch_object(mysqli_query($conn, "SELECT * FROM `ss_sales_executive` WHERE `id` = '".$excutive."' AND `published` = '1' AND `status` = '1' "));
$excut_name = !empty($excut_qry->name)?$excut_qry->name:'';
...
$html ='
    <p style="color:black; font-size:12px; text-align: center;">
        <strong style="color:green; font-size:18px; padding-bottom:1000px;">Yara Electronics (A Div of Cheenu Amma Alloy P Ltd )</strong><br/>
        <span>SF.No: 416/7, Thanneer Pandal,Vilankurichi Road, Peelamedu, Coimbatore - 641004<br>GSTIN\UIN : 33AADCC7429F1ZV<br></span>
...
$html ='<br><br>
    <table border= "1" cellpadding="1" top="100">
        <tr>
            <td rowspan="7" style="font-size:12px; width: 50%;">Shipped To: <br> '.$store.'<br> '.$owner.'<br> '.$address.'<br> '.$mobile.' Code:'.$state.'<br> GSTIN\UIN :'.$gst_number.'<br> TCS.No :'.$tcs_no.'</td>
        ...
        <tr>
            <td style="font-size:12px; width: 20%;">Sales Excutive</td>
            <td style="font-size:12px; width: 25%;">'.$excut_name.'</td>
        </tr>
    </table>
    ';
    ...
    <table border= "1" cellpadding="1" top="100">
        <thead>
            <tr>
                ...
                <td rowspan="2" style="font-size:12px; width: 20%;">Description</td>
        ...
        if($cou_2 > 0)
        {
            ...
            <tbody>';
                ...
                while($row_2 = mysqli_fetch_array($qry_2)) 
                {
                    ...
                    $gst_value    = !empty($row_2['gst'])?$row_2['gst']:'0';
                    ...
                    $html .= '
                        <tr>
                            ...
                            <td style="font-size:12px; width: 20%;">'.$code_val.'</td>
                            ...
                            {
                                $html .='<td style="font-size:12px; text-align: right; width: 12%;">'.round($gst_value).' %</td>
...
$html ='<br><br><br>
        <table border= "1" cellpadding="1">
            <tr>
                <th style="width:45%; font-size:12px;" colspan="3">
...
$pdf->Output($so_no.'_invoice.pdf', 'I');
// $pdf->Output($so_no.'_invoice.pdf', 'D');
