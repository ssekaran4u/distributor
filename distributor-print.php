...
$name     = isset($customer->owner)?$customer->owner:'---';
$address  = isset($customer->customeraddress)?$customer->customeraddress:'---';
$gst_no   = isset($customer->gst)?$customer->gst:'---';
...
$html ='<p style="color:black; font-size:12px; text-align: center;"><strong style="color:green; font-size:18px; padding-bottom:1000px;">Yara Electronics (A Div of Cheenu Amma Alloy P Ltd )</strong><br/>SF.No: 416/7, Thanneer Pandal,Vilankurichi Road, Peelamedu, Coimbatore - 641004<br>GSTIN\UIN : 33AADCC7429F1ZV<br><strong style="color:black; text-align:center; font-size:17px;"> TAX INVOICE</strong></p>';
...
<table border= "1" cellpadding="1" top="100">
        <tr>
            <td rowspan="6" style="font-size:12px; width: 50%;">Shipped To: <br>'.$store.'<br>'.$name.'<br>'.$address.'<br>'.$mobile.'<br>'.$s_name.', Code:'.$state.'<br>GSTIN\UIN :'.$gst_no.'<br>TCS.No :'.$tcs_no.'</td>
...
$html ='<br><br><table border= "1" cellpadding="1" top="100">
        <tr>
            ...
            <td rowspan="2" style="font-size:12px; width: 10%;">Description</td>
            <td rowspan="2" style="font-size:12px; text-align: center; width: 6%;">HSN</td>
            <td rowspan="2" style="font-size:12px; text-align: center; width: 6%;">Price</td>
            <td rowspan="2" style="font-size:12px; text-align: center; width: 5%;">Qty</td>
            <td rowspan="2" style="font-size:12px; text-align: center; width: 5%;">B A</td>
        ...
        if($_coun > 0)
        {
            ...
            while($_RES = mysqli_fetch_object($_QRY))
            {
                ...
                $serial = mysqli_query($conn, "SELECT * FROM `ss_item_stk` WHERE `product_id` = '".$_RES->pid."' AND `sales_no` = '".$_RES->so_no."' AND published = '1'");
                ...
                $html .= '
                    <tr>
                        ...
                        <td style="font-size:12px; width: 10%;">'.$_RES->code.'</td>
                        <td style="font-size:12px; text-align: center; width: 6%;">'.$_RES->hsn.'</td>
                        <td style="font-size:12px; text-align: right; width: 6%;">'.$_RES->price.'</td>
                        <td style="font-size:12px; text-align: center; width: 5%;">'.$_RES->qty.'</td>
                        <td style="font-size:12px; text-align: center; width: 5%;">'.round($allowance_val).'</td>
