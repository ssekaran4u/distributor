...
$sorts = array('title','status');

$nsort = $sorts[$sort];

$start_from = ($page-1) * $num_rec_per_page;

$qry = "SELECT * FROM `ss_slider` WHERE `title` != '' AND `published` = '1' ";
...
$qry .= " ORDER BY `ss_slider`.`".$nsort."` ".$order." LIMIT $start_from, $num_rec_per_page";

$selectquery = mysqli_query($conn, $qry);

$numrows = mysqli_num_rows($selectquery);

$cats = "SELECT * FROM `ss_slider` WHERE `status` = 1";
...
?>
    ...
    $i = 1;
        ...
        <tr>
            ...
            <td><?php echo $row['title'] ? $row['title'] : 'Empty'; ?></td>

            <td><?php echo $row['image'] ? 'Yes' : 'Empty'; ?></td>
            ...
            <td class="text-center">
                ...
                <a href="create-banner?type=edit&id=<?php echo $row['id'];?>"><span class="btn btn-sm btn-info"> <i class="mdi mdi-pencil"></i>  Edit </span> </a>

                <a class="delete-banner" data-id="<?php echo $row['id'];?>"><span class="btn btn-sm btn-danger"> <i class="mdi mdi-window-close"></i> Delete </span> </a>
