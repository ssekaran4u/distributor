...
$qry = "SELECT * FROM `ss_delar_allowance` WHERE `published` = '1' ";
if($search)
{
    $qry .= " AND `description` LIKE '%".$search."%' ";
}
$qry .= " ORDER BY `ss_delar_allowance`.`".$nsort."` ".$order." LIMIT $start_from, $num_rec_per_page";
$selectquery = mysqli_query($conn, $qry);
$numrows = mysqli_num_rows($selectquery);
$cats = "SELECT * FROM `ss_delar_allowance` WHERE `published` = '1'";
if(isset($search))
{
...
?>
    ...
    $i = 1;
        ...
        <tr>
            ...
            <td class="text-center">
                <a href="<?php echo "create-d-allowance?id=".$row['id'];?>"><span class="btn btn-sm btn-warning"> <i class="mdi mdi-pencil"></i>  Edit </span> </a>
                ...
                <a><span class="delete-d-allowance btn btn-sm btn-danger" data-id="<?php echo $row['id'];?>"> <i class="mdi mdi-window-close"></i> Delete </span> </a>
            </td>
        </tr>
    <?php } ?>
    <tr>
