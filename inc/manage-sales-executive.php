...
$qry = "SELECT * FROM `ss_sales_executive` WHERE `published` = '1'";
if($search)
{
    $qry .= " AND `name` LIKE '%".$search."%' ";
}
$qry .= " ORDER BY `ss_sales_executive`.`".$nsort."` ".$order." LIMIT $start_from, $num_rec_per_page";
$selectquery = mysqli_query($conn, $qry);
$numrows = mysqli_num_rows($selectquery);
$cats = "SELECT * FROM `ss_sales_executive` WHERE `published` = '1'";
if(isset($search))
{
    ...
    $i = 1;
        ...
        <tr>
            ...
            <td class="text-center">
                ...
                <a href="<?php echo "edit-sales-executive?id=".$row['id'];?>"><span class="btn btn-sm btn-warning"> <i class="mdi mdi-pencil"></i>  Edit </span> </a>
