...
$sorts = array('state_name','id');
$nsort = $sorts[$sort];
$start_from = ($page-1) * $num_rec_per_page;
$qry = "SELECT * FROM `ss_state` WHERE `published` = '1' AND country_id = '1'";
if($search)
{
    $qry .= " AND `state_name` LIKE '%".$search."%' ";
}
$qry .= " ORDER BY `ss_state`.`".$nsort."` ".$order." LIMIT $start_from, $num_rec_per_page";
$selectquery = mysqli_query($conn, $qry);
$numrows = mysqli_num_rows($selectquery);
$cats = "SELECT * FROM `ss_state` WHERE `published` = '1'";
if(isset($search))
{
    $cats .= " AND `state_name` LIKE '%".$search."%' ";
}
...
?>
    ...
    $i = 1;
        ...
        <tr>
            ...
            <td><?php echo empty(!$row['country_id'] == '1') ? 'India':'---'; ?></td>
            <!-- <td> 
            ...
            <td class="text-center">
                <a href="<?php echo "create-state?stateid=".$row['id'];?>"><span class="btn btn-sm btn-warning"> <i class="mdi mdi-pencil"></i>  Edit </span> </a>
