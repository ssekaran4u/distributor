...
$sorts = array('id','owner','status');
$nsort = $sorts[$sort];
$start_from = ($page-1) * $num_rec_per_page;
$qry = "SELECT * FROM `ss_distributors` WHERE `deleted` = '1' ";

if($search)
{
    $qry .= " AND `owner` LIKE '%".$search."%' ";
}
...
$cats = "SELECT * FROM `ss_distributors` WHERE `deleted` = '1'";
if(isset($search))
{
    $cats .= " AND `owner` LIKE '%".$search."%' ";
}
// echo $cats; die();
...
?>
    ...
    $i = 1;
        ...
        $cid=$row['id'];
        $cname   = isset($row['owner'])?$row['owner']:'---';
        $cmobile = isset($row['mobile'])?$row['mobile']:'---';
        $cemail  = isset($row['email_id'])?$row['email_id']:'---';
        ...
        <tr id="<?php echo $row['id']; ?>">
            ...
            <td class="text-center">
                ...
                <a href="<?php echo "add-payment?id=".$row['id'];?>"><span class="btn btn-sm btn-info"> <i class="mdi mdi-pencil"></i>  Add </span> </a>
