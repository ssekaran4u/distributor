...
if(isset($_REQUEST['so_no']))	
{
	...
	$qry = mysqli_query($conn, "SELECT * FROM `ss_distributor_inv_details` WHERE so_no = '".$so_no."' AND customer_id = '".$customer_id."' AND published = '1' AND status = '1'");
	// echo "SELECT * FROM `ss_distributor_inv_details` WHERE so_no = '".$so_no."' AND customer_id = '".$customer_id."' AND published = '1' AND status = '1'";
	...
	{	
		...
		while($res = mysqli_fetch_object($qry))
		{
			...
			$customer = mysqli_fetch_object(mysqli_query($conn, "SELECT * FROM ss_distributors WHERE id = '".$customer_id."' AND deleted = '1' AND status = '1'"));		
			...
			$_d_dis = $res->price * $customer->d_allowance / 100;
			...
			?>
				<tr>
					...
					<th><?php echo isset($product->allowance)?$res->qty * $product->allowance:'0'.' %'; ?></th>
					<th><?php echo isset($product->sta)?$res->qty * $product->sta:'0'; ?></th>
					<th><?php echo isset($customer->d_allowance)?$res->qty * $customer->d_allowance:'0'.' %'; ?></th>
					...
					<th>
						...
						{
						?>
						<a href="<?php echo "distributor-invoice?auto_id=".$res->auto_id."&id=".$res->so_id;?>"><span class="btn btn-sm btn-info"> <i class="mdi mdi-pencil"></i>  Edit </span> </a>
