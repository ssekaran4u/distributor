...
$qry = "SELECT * FROM `ss_employee` WHERE `published` = '1'";
if($search)
{
    $qry .= " AND `name` LIKE '%".$search."%' ";
}
$qry .= " ORDER BY `ss_employee`.`".$nsort."` ".$order." LIMIT $start_from, $num_rec_per_page";
$selectquery = mysqli_query($conn, $qry);
$numrows = mysqli_num_rows($selectquery);
$cats = "SELECT * FROM `ss_employee` WHERE `published` = '1'";
if(isset($search))
{
