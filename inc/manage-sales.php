...
$qry = "SELECT * FROM `ss_sales` WHERE `status` = '1' ";
if($_SESSION['type'] =='2')
{
...
$qry .= " ORDER BY `ss_sales`.`".$nsort."` ".$order." LIMIT $start_from, $num_rec_per_page";
// echo $qry; die;
$selectquery = mysqli_query($conn, $qry);
$numrows = mysqli_num_rows($selectquery);
$cats = "SELECT * FROM `ss_sales` WHERE `status` = '1'";
if(isset($search))
{
    ...
    $i = 1;
        ...
        <tr id="<?php echo $row['id']; ?>" <?php echo $color; ?>>
            ...
            <td class="text-center">
                <a href="<?php echo "view-bill-summery?id=".$row['id'];?>"><span class="btn btn-sm btn-success"> <i class="mdi mdi-eye"></i>   </span> </a>

                <?php
                    if($row['published'] == 1)
                    {
                        ?>
                        <!-- <a href="<?php echo "create-sales?id=".$row['id'];?>"><span class="btn btn-sm btn-info"> <i class="mdi mdi-pencil"></i>   </span> </a> -->
