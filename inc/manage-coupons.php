...
$sorts = array('coupon_code','percentage','validity','status');

$nsort = $sorts[$sort];

$start_from = ($page-1) * $num_rec_per_page;

$qry = "SELECT * FROM `ss_coupon_code` WHERE `branch` = '".$branch[0]."'";

if($search)

{

    $qry .= " AND `coupon_code` LIKE '%".$search."%' ";

}

$qry .= " ORDER BY `ss_coupon_code`.`".$nsort."` ".$order." LIMIT $start_from, $num_rec_per_page";
...
$cats = "SELECT * FROM `ss_coupon_code` WHERE `branch` = '".$branch[0]."'";

if($search)

{

    $cats .= " AND `coupon_code` LIKE '%".$search."%' ";
    ...
    $i = 1;
        ...
        <tr>
            ...
            <td class="text-center">
                ...
                <a href="create-coupon?view=edit&id=<?php echo $row['id']; ?>"><span class="btn btn-sm btn-info"> <i class="mdi mdi-pencil"></i>  Edit </span> </a>
