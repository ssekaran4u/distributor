...
$qry = "SELECT * FROM `ss_category` WHERE `published` = '1' ";
if($search)
{
    $qry .= " AND `title` LIKE '%".$search."%' ";
}
$qry .= " ORDER BY `ss_category`.`".$nsort."` ".$order." LIMIT $start_from, $num_rec_per_page";
// echo $qry; die;
$selectquery = mysqli_query($conn, $qry);
$numrows = mysqli_num_rows($selectquery);
$cats = "SELECT * FROM `ss_category` WHERE `published` = '1'";
if(isset($search))
{
    ...
    $i = 1;
        ...
        <tr id="<?php echo $row['id']; ?>">
            ...
            <td class="text-center">
                ...
                <a href="<?php echo "edit-category?id=".$row['id'];?>"><span class="btn btn-sm btn-info"> <i class="mdi mdi-pencil"></i>  Edit </span> </a>
